...
def main():
    ...
    def _guard_body(command_path: Path) -> str:
        ...
        _guard_start = _guard_text.index("**Cursor host check (tier-1 closed; fail-open only past it).**")
        _python_start = _guard_text.index("python3 - <<'PY'\n", _guard_start) + len("python3 - <<'PY'\n")
        _python_end = _guard_text.index("\nPY", _python_start)
        return _guard_text[_python_start:_python_end]

    def _run_guard(command_path: Path, cwd: Path, extra_env=None):
        _guard_env = _neutral_env()
        _guard_env.pop("CREW_HOST", None)
        _guard_env.pop("CLAUDE_CODE_ENTRYPOINT", None)
        _guard_env.pop("CLAUDECODE", None)
        _guard_env.pop("CODEX_THREAD_ID", None)
        _guard_env.pop("CODEX_SANDBOX_NETWORK_DISABLED", None)
        _guard_env.pop("CLAUDE_PLUGIN_ROOT", None)
        if extra_env:
            _guard_env.update(extra_env)
        return subprocess.run(
            ["python3", "-"],
            input=_guard_body(command_path),
            capture_output=True,
            text=True,
            cwd=cwd,
            env=_guard_env,
    ...
    _guard_paths = (
        PROJECT_DIR / "commands" / "build.md",
        PROJECT_DIR / "commands" / "measure-twice.md",
    )
    with tempfile.TemporaryDirectory() as _guard_outside:
        _guard_outside_path = Path(_guard_outside)
        for _guard_path in _guard_paths:
            _cursor_guard = _run_guard(
                _guard_path, _guard_outside_path, {"CREW_HOST": "CuRsOr"}
            )
            if _cursor_guard.stdout.strip() == "host=cursor":
                log_pass(f"{_guard_path.name}: explicit cursor binding stops before import")
            else:
                log_fail(f"{_guard_path.name}: explicit cursor binding stops before import",
                         "host=cursor", _cursor_guard.stdout)

            for _host in ("claude", "codex"):
                _host_guard = _run_guard(
                    _guard_path, _guard_outside_path, {"CREW_HOST": _host}
                )
                if "host=cursor" not in _host_guard.stdout:
                    log_pass(f"{_guard_path.name}: {_host} binding proceeds")
                else:
                    log_fail(f"{_guard_path.name}: {_host} binding proceeds",
                             "no host=cursor", _host_guard.stdout)

            _unbound_guard = _run_guard(_guard_path, _guard_outside_path)
            if "host-check-inconclusive:" in _unbound_guard.stdout:
                log_pass(f"{_guard_path.name}: unbound outside guard fails open")
            else:
                log_fail(f"{_guard_path.name}: unbound outside guard fails open",
                         "host-check-inconclusive:", _unbound_guard.stdout)

            _root_guard = _run_guard(
                _guard_path,
                _guard_outside_path,
                {"CLAUDE_PLUGIN_ROOT": str(PROJECT_DIR)},
            )
            if (_root_guard.stdout.startswith("host=")
                    and "host-check-inconclusive" not in _root_guard.stdout):
                log_pass(f"{_guard_path.name}: environment root resolves from any cwd")
            else:
                log_fail(f"{_guard_path.name}: environment root resolves from any cwd",
                         "host=...", _root_guard.stdout)

        # This fixed-depth root assumes tests/test-hooks.py remains under plugins/crew/scripts/tests/.
        _repo_guard = _run_guard(_guard_paths[0], Path(__file__).resolve().parents[4])
        if _repo_guard.stdout.startswith("host=") and "host-check-inconclusive" not in _repo_guard.stdout:
            log_pass("build.md: unresolved placeholder resolves from repository cwd")
        else:
            log_fail("build.md: unresolved placeholder resolves from repository cwd",
                     "host=...", _repo_guard.stdout)

        _broken_root = _guard_outside_path / "broken"
        (_broken_root / "plugins" / "crew" / "scripts").mkdir(parents=True)
        (_broken_root / "plugins" / "crew" / "scripts" / "host_detect.py").write_text(
            "this is not valid python\n"
        )
        _broken_import = _run_guard(_guard_paths[0], _broken_root)
        if "host-check-inconclusive:" in _broken_import.stdout:
            log_pass("build.md: broken helper import fails open after tier one")
        else:
            log_fail("build.md: broken helper import fails open after tier one",
                     "host-check-inconclusive:", _broken_import.stdout)

        _tier_one = _run_guard(
            _guard_paths[0], _broken_root, {"CREW_HOST": "cursor"}
        )
        if _tier_one.stdout.strip() == "host=cursor":
            log_pass("build.md: tier one beats a broken helper import")
        else:
            log_fail("build.md: tier one beats a broken helper import",
                     "host=cursor", _tier_one.stdout)
