...
git/inspection) plus `Write` scoped to `.crew/reviews/<session-id>/` for its return
file. Because every seat's prompt comes from the one `render` source over the same
`working-tree` target, the panel reviews a single identical diff.
