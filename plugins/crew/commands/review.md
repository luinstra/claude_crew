...
- `--panel full|lite|solo|cursor|quick|<custom>` — a named preset. The five
  ...
  per-repo or global config (e.g. `[panels].nightly`). `--seats <comma-list>` —
  an explicit subset of any registered seat (e.g. `--seats codex,opus`);
  `--seats` wins if both are given. `cursor-gemini`, `cursor-glm`, and `cursor-gpt` are opt-in (add via `--seats`).
...
the printed path is the `<seat>.json` the Step-5.5 collect reads. The seat
